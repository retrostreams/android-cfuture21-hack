...
/**
 * An {@link ExecutorService} for running {@link FJTask}s.
 * A {@code FJPool} provides the entry point for submissions
 * from non-{@code FJTask} clients, as well as management and
 * monitoring operations.
 *
 * <p>A {@code FJPool} differs from other kinds of {@link
 * ExecutorService} mainly by virtue of employing
 * <em>work-stealing</em>: all threads in the pool attempt to find and
 * execute tasks submitted to the pool and/or created by other active
 * tasks (eventually blocking waiting for work if none exist). This
 * enables efficient processing when most tasks spawn other subtasks
 * (as do most {@code FJTask}s), as well as when many small
 * tasks are submitted to the pool from external clients.  Especially
 * when setting <em>asyncMode</em> to true in constructors, {@code
 * FJPool}s may also be appropriate for use with event-style
 * tasks that are never joined. All worker threads are initialized
 * with {@link Thread#isDaemon} set {@code true}.
 *
 * <p>A static {@link #commonPool()} is available and appropriate for
 * most applications. The common pool is used by any FJTask that
 ...
 * FJPool} may be constructed with a given target parallelism
 ...
 * forms of these methods accept instances of {@code FJTask},
 ...
 * @author Doug Lea
 */
class FJPool extends AbstractExecutorService {
// CVS rev. 1.344
    /*
     ...
     * FJWorkerThread via its factory. Upon construction, the
     ...
     * in FJWorkerThread) may be JVM-dependent and must access
     ...
     * FJPool, FJWorkerThread, and FJTask.  The
     * fields of WorkQueue maintain data structures managed by
     * FJPool, so are directly accessed.  There is little point
     ...
     * (6) Callbacks and other support for FJTask methods
     * (7) Exported methods
    ...
    /**
     * Factory for creating new {@link FJWorkerThread}s.
     * A {@code ForkJoinWorkerThreadFactory} must be defined and used
     * for {@code FJWorkerThread} subclasses that extend base
    ...
    static interface ForkJoinWorkerThreadFactory {
         ...
         */
        public FJWorkerThread newThread(FJPool pool);
    }
     ...
     * new FJWorkerThread using the system class loader as the
    ...
    private static final class DefaultForkJoinWorkerThreadFactory
        ...
        public final FJWorkerThread newThread(FJPool pool) {
            return AccessController.doPrivileged(
                new PrivilegedAction<FJWorkerThread>() {
                    public FJWorkerThread run() {
                        return new FJWorkerThread(
    ...
    // Constants shared across FJPool and WorkQueue
    ...
    static final class WorkQueue {
        ...
        FJTask<?>[] array;   // the elements (initially unallocated)
        final FJPool pool;   // the containing pool (may be null)
        final FJWorkerThread owner; // owning thread or null if shared
        ...
        final FJTask<?>[] growArray() {
            ...
            int oldMask, t, b;
            FJTask<?>[] a = array = new FJTask<?>[size];
        ...
        for (;;) {
            ...
            if ((md & SHUTDOWN) != 0 || ws == null || (n = ws.length) <= 0)
                ...
                if ((q = ws[(n - 1) & r & SQMASK]) == null) {
                    ...
                    int b = q.base, s = q.top, al, d; FJTask<?>[] a;
    ...
    public void execute(Runnable task) {
        Utils.requireNonNull(task);
        FJTask<?> job;
        if (task instanceof FJTask<?>) // avoid re-wrap
            job = (FJTask<?>) task;
        else
    ...
    public <T> FJTask<T> submit(Callable<T> task) {
        return externalSubmit(new FJTask.AdaptedCallable<T>(task));
    }
    ...
    public <T> FJTask<T> submit(Runnable task, T result) {
        return externalSubmit(new FJTask.AdaptedRunnable<T>(task, result));
    }
    ...
    public FJTask<?> submit(Runnable task) {
        Utils.requireNonNull(task);
        return externalSubmit((task instanceof FJTask<?>)
            ? (FJTask<Void>) task // avoid re-wrap
            : new FJTask.AdaptedRunnableAction(task));
    }
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) {
        ...
        // a task to run FJTask.invokeAll, but now external
        // invocation of multiple tasks is at least as efficient.
        ArrayList<Future<T>> futures = new ArrayList<>(tasks.size());

        try {
            for (Callable<T> t : tasks) {
                FJTask<T> f = new FJTask.AdaptedCallable<T>(t);
                futures.add(f);
                externalSubmit(f);
            }
    ...
    static void helpAsyncBlocker(Executor e, ManagedBlocker blocker) {
        if (blocker != null && (e instanceof FJPool)) {
            ...
            if (w != null) {
                for (;;) {
                    int b = w.base, s = w.top, d, al; FJTask<?>[] a;
    ...
    // fact that FJTask.adapt returns FJTasks that also
    ...
    static {
        try {
            CTL = U.objectFieldOffset
                (FJPool.class.getDeclaredField("ctl"));
            MODE = U.objectFieldOffset
                (FJPool.class.getDeclaredField("mode"));
            ABASE = U.arrayBaseOffset(FJTask[].class);
            int scale = U.arrayIndexScale(FJTask[].class);
            if ((scale & (scale - 1)) != 0)
                throw new ExceptionInInitializerError("array index scale not a power of two");
