 ...
 * argument are performed using the {@link FJPool#commonPool()}
...
// CVS rev. 1.221
     ...
     * Class Completion extends FJTask to enable async execution
    ...
    private static final boolean USE_COMMON_POOL =
        (FJPool.getCommonPoolParallelism() > 1);

    /**
     * Default executor -- FJPool.commonPool() unless it cannot
     * support parallelism.
     */
    private static final Executor ASYNC_POOL = USE_COMMON_POOL ?
        FJPool.commonPool() : new ThreadPerTaskExecutor();

    /** Fallback if FJPool.commonPool() cannot support parallelism */
    ...
    static Executor screenExecutor(Executor e) {
        if (!USE_COMMON_POOL && e == FJPool.commonPool())
            return ASYNC_POOL;
        return Utils.requireNonNull(e);
    }
    ...
    abstract static class Completion extends FJTask<Void>
    ...
    private CompletableFuture<Void> biRunStage(Executor e,
    ...
    private CompletableFuture<Void> orRunStage(Executor e,
            CompletionStage<?> o, Runnable f) {
        CompletableFuture<?> b;
        if (f == null || (b = o.toCompletableFuture()) == null)
            throw new NullPointerException();
    ...
    @SuppressWarnings("serial")
    static final class AsyncSupply<T> extends FJTask<Void>
        implements Runnable, AsynchronousCompletionTask {
    ...
    @SuppressWarnings("serial")
    static final class AsyncRun extends FJTask<Void>
        implements Runnable, AsynchronousCompletionTask {
     ...
     * blocking actions pile up in FJPools.
     */
    @SuppressWarnings("serial")
    static final class Signaller extends Completion
        implements FJPool.ManagedBlocker {
     ...
     * by a task running in the {@link FJPool#commonPool()} with
    ...
    /**
     * Returns a new CompletableFuture that is asynchronously completed
     * by a task running in the {@link FJPool#commonPool()} after
    ...
    public boolean completeExceptionally(Throwable ex) {
        boolean triggered = internalComplete(new AltResult(Utils.requireNonNull(ex)));
        postComplete();
        return triggered;
    }
     ...
     */
    public void obtrudeException(Throwable ex) {
        result = new AltResult(Utils.requireNonNull(ex));
        postComplete();
    }
     ...
     * FJPool#commonPool()} if it supports more than one
     ...
     */
    public static Executor delayedExecutor(long delay, TimeUnit unit) {
        return new DelayedExecutor(delay, Utils.requireNonNull(unit), ASYNC_POOL);
    }
    ...
    public static <U> CompletableFuture<U> failedFuture(Throwable ex) {
        return new CompletableFuture<U>(new AltResult(Utils.requireNonNull(ex)));
    }
    ...
    public static <U> CompletionStage<U> failedStage(Throwable ex) {
        return new MinimalStage<U>(new AltResult(Utils.requireNonNull(ex)));
    }
    ...
    static final class Canceller implements BiConsumer<Object, Throwable> {
        ...
        public void accept(Object ignore, Throwable ex) {
            if (ex == null && (f instanceof FJTask && !((FJTask<?>) f).isDone())) {
                f.cancel(false);
            }
        }
    }
