...
/**
 * A {@link FJTask} with a completion action performed when
 * triggered and there are no remaining pending actions.
 * CountedCompleters are in general more robust in the
 * presence of subtask stalls and blockage than are other forms of
 * ForkJoinTasks, but are less intuitive to program.  Uses of
 * CntdCompleter are similar to those of other completion based
 * components (such as {@link java.nio.channels.CompletionHandler})
 * except that multiple <em>pending</em> completions may be necessary
 * to trigger the completion action {@link #onCompletion(CntdCompleter)},
 * not just one.
 ...
 * components such as {@link java.util.concurrent.Phaser} and
 ...
 *
 * <p>A concrete CntdCompleter class must define method {@link
 * #compute}, that should in most cases (as illustrated below), invoke
 * {@code tryComplete()} once before returning. The class may also
 * optionally override method {@link #onCompletion(CntdCompleter)}
 * to perform an action upon normal completion, and method
 * {@link #onExceptionalCompletion(Throwable, CntdCompleter)} to
 * perform an action upon any exception.
 *
 * <p>CountedCompleters most often do not bear results, in which case
 * they are normally declared as {@code CntdCompleter<Void>}, and
 * will always return {@code null} as a result value.  In other cases,
 * you should override method {@link #getRawResult} to provide a
 * result from {@code join(), invoke()}, and related methods.  In
 * general, this method should return the value of a field (or a
 * function of one or more fields) of the CntdCompleter object that
 * holds the result upon completion. Method {@link #setRawResult} by
 * default plays no role in CountedCompleters.  It is possible, but
 * rarely applicable, to override this method to maintain other
 * objects or fields holding result data.
 *
 * <p>A CntdCompleter that does not itself have a completer (i.e.,
 * one for which {@link #getCompleter} returns {@code null}) can be
 * used as a regular ForkJoinTask with this added functionality.
 * However, any completer that in turn has another completer serves
 * only as an internal helper for other computations, so its own task
 * status (as reported in methods such as {@link FJTask#isDone})
 * is arbitrary; this status changes only upon explicit invocations of
 * {@link #complete}, {@link FJTask#cancel},
 * {@link FJTask#completeExceptionally(Throwable)} or upon
 * exceptional completion of method {@code compute}. Upon any
 * exceptional completion, the exception may be relayed to a task's
 * completer (and its completer, and so on), if one exists and it has
 * not otherwise already completed. Similarly, cancelling an internal
 * CntdCompleter has only a local effect on that completer, so is
 * not often useful.
 *
 * <p><b>Sample Usages.</b>
 *
 ...
 * implicitly using the {@link FJPool#commonPool()}).  It is
 ...
 *   class Task extends CntdCompleter<Void> {
 *     final int lo, hi;
 ...
 * in this tree implements an {@link #onCompletion(CntdCompleter)} method,
 ...
 *
 * <pre> {@code
 * class Searcher<E> extends CntdCompleter<E> {
 *   final E[] array; final AtomicReference<E> result; final int lo, hi;
 *   Searcher(CntdCompleter<?> p, E[] array, AtomicReference<E> result, int lo, int hi) {
 *     super(p);
 ...
 *
 * <p><b>Recording subtasks.</b> CntdCompleter tasks that combine
 * results of multiple subtasks usually need to access these results
 * in method {@link #onCompletion(CntdCompleter)}. As illustrated in the following
 ...
 * class MapReducer<E> extends CntdCompleter<E> {
 *   final E[] array; final MyMapper<E> mapper;
 *   final MyReducer<E> reducer; final int lo, hi;
 *   MapReducer<E> sibling;
 *   E result;
 *   MapReducer(CntdCompleter<?> p, E[] array, MyMapper<E> mapper,
 ...
 *     }
 *   }
 *   public void onCompletion(CntdCompleter<?> caller) {
 *     if (caller != this) {
 ...
 *
 * <pre> {@code
 * class MapReducer<E> extends CntdCompleter<E> { // version 2
 *   final E[] array; final MyMapper<E> mapper;
 *   final MyReducer<E> reducer; final int lo, hi;
 ...
 *     for (CntdCompleter<?> c = firstComplete(); c != null; c = c.nextComplete()) {
 ...
 *
 * <pre> {@code
 * class HeaderBuilder extends CntdCompleter<...> { ... }
 * class BodyBuilder extends CntdCompleter<...> { ... }
 * class PacketSender extends CntdCompleter<...> {
 *   PacketSender(...) { super(null, 1); ... } // trigger on second completion
 *   public void compute() { } // never called
 *   public void onCompletion(CntdCompleter<?> caller) { sendPacket(); }
 * }
 * // sample use:
...
// CVS rev. 1.64
    ...
    /**
     * Creates a new CntdCompleter with the given completer
     * and initial pending count.
     *
     * @param completer this task's completer, or {@code null} if none
     * @param initialPendingCount the initial pending count
     */
    protected CntdCompleter(CntdCompleter<?> completer,
     ...
     * be this task itself)
     */
    void onCompletion(CntdCompleter<?> caller) {
    }
     ...
     */
    boolean onExceptionalCompletion(Throwable ex, CntdCompleter<?> caller) {
        return true;
    }
     ...
     * otherwise invokes {@link #onCompletion(CntdCompleter)}
     * and then similarly tries to complete this task's completer,
     * if one exists, else marks this task as complete.
     */
    final void tryComplete() {
        CntdCompleter<?> a = this, s = a;
        for (int c;;) {
            if ((c = a.pending) == 0) {
                a.onCompletion(s);
     ...
     * #onCompletion(CntdCompleter)} along the completion path:
     * If the pending count is nonzero, decrements the count;
     ...
     * {@link #onCompletion(CntdCompleter)}, marks this task as
     * complete and further triggers {@link #tryComplete} on this
     * task's completer, if one exists.  The given rawResult is
     * used as an argument to {@link #setRawResult} before invoking
     * {@link #onCompletion(CntdCompleter)} or marking this task
     ...
     *
     * <pre> {@code
     * for (CntdCompleter<?> c = firstComplete();
     *      c != null;
     *      c = c.nextComplete()) {
     *   // ... process c ...
     * }}</pre>
    ...
    final void helpComplete(int maxTasks) {
        ...
        if (maxTasks > 0 && status >= 0) {
            if ((t = Thread.currentThread()) instanceof FJWorkerThread)
                ...
                FJPool.common.externalHelpComplete(this, maxTasks);
        }
    }
    ...
    static {
        try {
            PENDING = U.objectFieldOffset
                (CntdCompleter.class.getDeclaredField("pending"));
        } catch (Exception e) {
